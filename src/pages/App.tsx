...
const IndexPage = (initialState: StateType, ipResult: apiResultType) => {
  ...
  let onSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if(values.link !== undefined && values.link.length !== 0) {
      setLoading(true)

      axios.get(`http://ipwhois.app/json/${values.link}?lang=en`)
      .then((res) => {
  ...
  return (
    <div className="min-h-screen container mx-auto py-4 bg-grey-100" style={ maxWidth }>
      ...
      <form onSubmit={onSubmit}>
        ...
        <div className="mt-5">
          <button
            className="w-full flex flex-row justify-center items-center p-3 bg-indigo-900 text-white rounded-lg shadow-lg focus:outline-none active:bg-indigo-800 disabled:opacity-80"
            type="submit"
          >
